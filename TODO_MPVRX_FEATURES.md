...
| 6 | ✅ **Dual subtitle + speech-to-subtitle (Whisper)** | Trung bình/Cao | Done — dual sub qua mpv `secondary-sid` (`PlayerDialogManager.showSecondarySubtitleDialog`); speech-to-subtitle ở `ai/transcribe/` (`AudioChunkExtractor` decode audio bằng MediaExtractor/MediaCodec → WAV 16kHz mono chunk 5 phút → OpenAI/Groq `audio/transcriptions` với `response_format=srt` → ghép offset). UI: `TranscribeSubtitleDialog` trong menu subtitle của player. **Cloud-only**: mpv build của app không có whisper.cpp, on-device cần thêm native dep + rebuild `.so` cho 4 ABI. Container nào chạy được phụ thuộc `MediaExtractor` của máy (MP4/MKV/WebM/TS ok, AVI thường không) |
