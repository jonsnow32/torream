...
import com.tv.apps.zippy.ui.dialog.FeedActionDialog
import com.tv.apps.zippy.ui.dialog.ActionItem
...
class FeedAction(
  ...
  private val playlistRepository: com.tv.apps.zippy.media.repository.PlaylistRepository
) {
                    ...
                    fragment.lifecycleScope.launch {
                      try {
                        ...
                        try {
                          ...
                          if (activity is com.tv.apps.zippy.MainActivity) {
...
//      )
  ...
  private fun handleMediaItemAction(
    ...
    when (actionItem.id) {
      ...
      "add_to_favorite" -> {
        fragment.lifecycleScope.launch {
          try {
            val favorite = com.tv.apps.zippy.media.dao.FavoriteEntity(
              id = item.media.uri,
              type = "media",
              title = item.title,
              uri = item.media.uri,
      ...
      "delete" -> {
        fragment.lifecycleScope.launch {
          try {
            ...
            try {
              (fragment as? com.tv.apps.zippy.ui.library.LibraryFragment)?.let { frag ->
            ...
            try {
              (fragment as? com.tv.apps.zippy.ui.home.FeedFragment)?.refreshAdapter()
            } catch (_: Exception) {
            }
          } catch (e: Exception) {
  ...
  fun playDownloadedFile(item: FeedData, state: com.tv.apps.zippy.download.DownloadState) {
    try {
      val context = fragment.context ?: run {
        showToast("Context not available")
        return
  ...
  private fun playPlaylistItems(items: List<com.tv.apps.zippy.media.entities.PlaylistItemEntity>) {
    fragment.viewLifecycleOwner.lifecycleScope.launch {
      try {
        if (items.isEmpty()) {
