...
class InAppUpdater {
  companion object {
    ...
    private suspend fun Activity.getPreReleaseUpdate(): Update {
      ...
      val prePreleaseResponseJson = app.get(tagUrl, headers = headers).text

      val tagResponse = parseJson<GithubTag>(prePreleaseResponseJson)
