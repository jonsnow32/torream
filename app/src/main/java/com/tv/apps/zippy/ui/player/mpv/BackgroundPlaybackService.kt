...
import com.tv.apps.zippy.MainActivity
...
class BackgroundPlaybackService : Service(), MPVLib.EventObserver {
    ...
    private var cachedMetadata = MPVUtils.AudioMetadata()
    ...
    private fun buildNotification(): Notification {
        ...
        val playPauseAction = if (paused)
            buildNotificationAction(R.drawable.ic_play_arrow_black_24dp, R.string.play_with_app_name, "PLAY_PAUSE")
        else
            buildNotificationAction(R.drawable.ic_pause_black_24dp, R.string.pause, "PLAY_PAUSE")

        val style = MediaStyle()
        mediaToken?.let { style.setMediaSession(it) }
        if (shouldShowPrevNext) {
            builder.addAction(buildNotificationAction(
                R.drawable.ic_skip_previous_black_24dp, R.string.previous, "ACTION_PREV"
            ))
            builder.addAction(playPauseAction)
            builder.addAction(buildNotificationAction(
                R.drawable.ic_skip_next_black_24dp, R.string.next, "ACTION_NEXT"
            ))
