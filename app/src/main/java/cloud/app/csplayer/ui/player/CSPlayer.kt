...
import cloud.app.csplayer.utils.DrmExtractorLink
import cloud.app.csplayer.utils.ExtractorLink
import cloud.app.csplayer.utils.ExtractorLinkPlayList
import cloud.app.csplayer.utils.ExtractorLinkType
import cloud.app.csplayer.utils.ExtractorUri
import cloud.app.csplayer.utils.PlayerSubtitleHelper
...
//            isPlaying,
  ...
  companion object {
    ...
    var preferredAudioTrackLanguage: String? = null
      get() {
        return field ?: Utils.activity?.getKey(
      ...
      set(value) {
        Utils.activity?.setKey("$PREFERRED_AUDIO_LANGUAGE_KEY", value)
        field = value
      }
    ...
    private fun getCache(context: Context, cacheSize: Long): SimpleCache? {
      return try {
        ...
        SimpleCache(
          ...
          ).also { context.deleteFileOnExit(it) }, // Ensures always fresh file
  ...
  private fun getCurrentTimestamp(writePosition: Long? = null): SkipStamp? {
    val position = writePosition ?: this@CSPlayer.getPosition() ?: return null
  ...
  private var lastTimeStamps: List<SkipStamp> = emptyList()

  @SuppressLint("UnsafeOptInUsageError")
  override fun addTimeStamps(timeStamps: List<SkipStamp>) {
