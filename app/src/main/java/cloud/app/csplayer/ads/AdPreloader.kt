package cloud.app.csplayer.ads
...
class AdPreloader @Inject constructor(
    ...
    data class CachedAd(
        val adType: cloud.app.csplayer.ads.providers.AdProvider.AdType,
        val timestamp: Long,
        val providerType: cloud.app.csplayer.ads.providers.AdProvider.ProviderType
    )
    ...
    private suspend fun preloadAdsIfNeeded(context: Context) {
        ...
        if (shouldPreload("interstitial", currentTime)) {
            preloadAdType(context, cloud.app.csplayer.ads.providers.AdProvider.AdType.INTERSTITIAL, "interstitial")
        }

        // Kiểm tra và preload rewarded ads
        if (shouldPreload("rewarded", currentTime)) {
            preloadAdType(context, cloud.app.csplayer.ads.providers.AdProvider.AdType.REWARDED, "rewarded")
        }
    }
    ...
    private suspend fun preloadAdType(
        context: Context,
        adType: cloud.app.csplayer.ads.providers.AdProvider.AdType,
        adKey: String
    ) {
    ...
    suspend fun preloadAdImmediately(
        ...
        val adKey = when (adType) {
            cloud.app.csplayer.ads.providers.AdProvider.AdType.INTERSTITIAL -> "interstitial"
            cloud.app.csplayer.ads.providers.AdProvider.AdType.REWARDED -> "rewarded"
            else -> return false
        }
     ...
     */
    fun isAdReady(adType: cloud.app.csplayer.ads.providers.AdProvider.AdType): Boolean {
        val adKey = when (adType) {
            cloud.app.csplayer.ads.providers.AdProvider.AdType.INTERSTITIAL -> "interstitial"
            cloud.app.csplayer.ads.providers.AdProvider.AdType.REWARDED -> "rewarded"
            else -> return false
     ...
     */
    fun getCachedAdProvider(adType: cloud.app.csplayer.ads.providers.AdProvider.AdType): cloud.app.csplayer.ads.providers.AdProvider.ProviderType? {
        val adKey = when (adType) {
            cloud.app.csplayer.ads.providers.AdProvider.AdType.INTERSTITIAL -> "interstitial"
            cloud.app.csplayer.ads.providers.AdProvider.AdType.REWARDED -> "rewarded"
            else -> return null
