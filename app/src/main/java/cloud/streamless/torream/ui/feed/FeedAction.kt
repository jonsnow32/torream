...
import cloud.streamless.torream.ui.dialog.FeedActionDialog
import cloud.streamless.torream.ui.dialog.ActionItem
...
class FeedAction(
  ...
  private val favoriteRepository: cloud.streamless.torream.favorites.FavoriteRepository,
                    ...
                    fragment.lifecycleScope.launch {
                      try {
                        ...
                        try {
                          ...
                          if (activity is cloud.streamless.torream.MainActivity) {
...
//      )
  ...
  private fun handleMediaItemAction(
    ...
    when (actionItem.id) {
      ...
      "add_to_favorite" -> {
        fragment.lifecycleScope.launch {
          try {
            val favorite = cloud.streamless.torream.media.dao.FavoriteEntity(
              id = item.media.uri,
              type = "media",
              title = item.title,
              uri = item.media.uri,
      ...
      "delete" -> {
        fragment.lifecycleScope.launch {
          try {
            ...
            try {
              (fragment as? cloud.streamless.torream.ui.library.LibraryFragment)?.let { frag ->
  ...
  fun playDownloadedFile(item: FeedData, state: cloud.streamless.torream.download.DownloadState) {
    try {
      val context = fragment.context ?: run {
        showToast("Context not available")
        return
  ...
  private fun playPlaylistItems(items: List<cloud.streamless.torream.media.entities.PlaylistItemEntity>) {
    fragment.viewLifecycleOwner.lifecycleScope.launch {
      try {
        if (items.isEmpty()) {
