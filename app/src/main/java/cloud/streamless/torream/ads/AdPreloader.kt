package cloud.streamless.torream.ads

import android.content.Context
import cloud.streamless.torream.ads.providers.AdProvider
...
class AdPreloader @Inject constructor(
  ...
  data class CachedAd(
    val adType: AdProvider.AdType,
    val timestamp: Long,
    val providerType: AdProvider.ProviderType
  )
  ...
  private suspend fun preloadAdsIfNeeded(context: Context) {
    ...
    if (shouldPreload("interstitial", currentTime)) {
      preloadAdType(context, AdProvider.AdType.INTERSTITIAL, "interstitial")
    }

    // Kiểm tra và preload rewarded ads
    if (shouldPreload("rewarded", currentTime)) {
      preloadAdType(context, AdProvider.AdType.REWARDED, "rewarded")
    }
  }
  ...
  suspend fun preloadAdImmediately(
    ...
    val adKey = when (adType) {
      AdProvider.AdType.INTERSTITIAL -> "interstitial"
      AdProvider.AdType.REWARDED -> "rewarded"
      else -> return false
    }
   ...
   */
  fun getCachedAdProvider(adType: AdProvider.AdType): AdProvider.ProviderType? {
    val adKey = when (adType) {
      AdProvider.AdType.INTERSTITIAL -> "interstitial"
      AdProvider.AdType.REWARDED -> "rewarded"
      else -> return null
